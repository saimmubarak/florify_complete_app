...
def handler(event, context):
    ...
    try:
        ...
        if pipeline_png_with_skins and pipeline_png_with_skins.startswith('data:'):
            s3_key = f"blueprints/{user_id}/{garden_id}/{blueprint_id}/pipeline-with-skins.png"
            s3_url = upload_image_to_s3(pipeline_png_with_skins, s3_key)
            if s3_url:
                update_expression += ", pipelinePngWithSkinsUrl = :pipelinePngWithSkinsUrl"
                expression_values[':pipelinePngWithSkinsUrl'] = s3_url
                print(f"Uploaded 512×512 pipeline PNG with skins to: {s3_url}")
